export interface ExperienceInterface {
  key: string;
  companyId: string;
  companyName: string;
  year: string;
  mainStack: string[];
  resume: boolean;
  resumeAccomplishments: number[];
  hasBlackLogo: boolean;
}

export const experience: ExperienceInterface[] = [
  {
    key: "danielxavier-1",
    companyId: "danielxavier",
    companyName: "<DanielXavier />",
    year: "2024",
    mainStack: [
      "react",
      "next",
      "tailwind",
      "node",
      "nest",
      "ts",
      "kong",
      "jest",
      "cypress",
      "html",
      "css",
      "js",
      "docker",
      "mysql",
    ],
    resume: true,
    resumeAccomplishments: [1, 2, 3],
    hasBlackLogo: true,
  },
  {
    key: "olx-4",
    companyId: "olx",
    companyName: "OLX",
    year: "2024",
    mainStack: [
      "react",
      "node",
      "kong",
      "s3",
      "firebase",
      "android",
      "kotlin",
      "ios",
      "swift-ui",
      "rn",
    ],
    resume: true,
    resumeAccomplishments: [1, 2, 4],
    hasBlackLogo: false,
  },
  {
    key: "olx-3",
    companyId: "olx",
    companyName: "OLX",
    year: "2023",
    mainStack: [
      "python",
      "es",
      "react",
      "storybook",
      "s3",
      "androind",
      "kotlin",
      "ios",
      "swift-ui",
      "rn",
      "postgre",
    ],
    resume: true,
    resumeAccomplishments: [1, 2, 3, 4],
    hasBlackLogo: false,
  },
  {
    key: "olx-2",
    companyId: "olx",
    companyName: "OLX",
    year: "2022 ~ 2023",
    mainStack: [
      "react",
      "storybook",
      "s3",
      "android",
      "kotlin",
      "ios",
      "swift-ui",
    ],
    resume: true,
    resumeAccomplishments: [1, 3, 5],
    hasBlackLogo: false,
  },
  {
    key: "olx-1",
    companyId: "olx",
    companyName: "OLX",
    year: "2021 ~ 2022",
    mainStack: [
      "html",
      "css",
      "js",
      "s3",
      "firebase",
      "android",
      "java",
      "kotlin",
      "ios",
      "swift-ui",
      "ad-manager",
    ],
    resume: true,
    resumeAccomplishments: [1, 2, 3],
    hasBlackLogo: false,
  },
  {
    key: "meaning-1",
    companyId: "meaning",
    companyName: "Meaning",
    year: "2023",
    mainStack: ["html", "css", "js", "bpmn"],
    resume: true,
    resumeAccomplishments: [1, 2, 3],
    hasBlackLogo: false,
  },
  {
    key: "softplan-3",
    companyId: "softplan",
    companyName: "Softplan",
    year: "2020 ~ 2021",
    mainStack: [
      "react",
      "micro-frontend",
      "dotnet",
      "graphql",
      "delphi",
      "sql-server",
    ],
    resume: true,
    resumeAccomplishments: [1, 2, 3],
    hasBlackLogo: false,
  },
  {
    key: "softplan-2",
    companyId: "softplan",
    companyName: "Softplan",
    year: "2019 ~ 2020",
    mainStack: ["react", "micro-frontend", "dotnet", "graphql", "sql-server"],
    resume: true,
    resumeAccomplishments: [1, 2, 4],
    hasBlackLogo: false,
  },
  {
    key: "softplan-1",
    companyId: "softplan",
    companyName: "Softplan",
    year: "2018 ~ 2019",
    mainStack: ["react", "micro-frontend", "dotnet", "graphql", "sql-server"],
    resume: true,
    resumeAccomplishments: [1, 2, 3],
    hasBlackLogo: false,
  },
  {
    key: "hotmart-3",
    companyId: "hotmart",
    companyName: "Hotmart",
    year: "2018",
    mainStack: ["vue", "nuxt", "node", "mysql", "html", "css", "js"],
    resume: true,
    resumeAccomplishments: [1, 2],
    hasBlackLogo: false,
  },
  {
    key: "hotmart-2",
    companyId: "hotmart",
    companyName: "Hotmart",
    year: "2017 ~ 2018",
    mainStack: [
      "vue",
      "nuxt",
      "node",
      "mysql",
      "html",
      "css",
      "js",
      "php",
      "wordpress",
      "nginx",
    ],
    resume: true,
    resumeAccomplishments: [1, 2],
    hasBlackLogo: false,
  },
  {
    key: "hotmart-1",
    companyId: "hotmart",
    companyName: "Hotmart",
    year: "2016 ~ 2017",
    mainStack: ["html", "css", "js", "php", "wordpress", "nginx", "mysql"],
    resume: false,
    resumeAccomplishments: [],
    hasBlackLogo: false,
  },
  {
    key: "sydle-2",
    companyId: "sydle",
    companyName: "Sydle",
    year: "2014 ~ 2016",
    mainStack: [
      "node",
      "express",
      "loopback",
      "angular",
      "html",
      "css",
      "js",
      "php",
      "wordpress",
      "magento",
      "apache",
      "bpmn",
      "linux",
      "windows",
      "oracle",
      "sql-server",
      "mysql",
    ],
    resume: false,
    resumeAccomplishments: [],
    hasBlackLogo: true,
  },
  {
    key: "sydle-1",
    companyId: "sydle",
    companyName: "Sydle",
    year: "2012 ~ 2014",
    mainStack: ["js", "bpmn", "linux", "windows", "oracle", "sql-server"],
    resume: false,
    resumeAccomplishments: [],
    hasBlackLogo: true,
  },
];
